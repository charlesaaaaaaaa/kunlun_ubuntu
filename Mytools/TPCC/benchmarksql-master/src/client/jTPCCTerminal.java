...
public class jTPCCTerminal implements jTPCCConfig, Runnable
{
    ...
    public void run()
    {
        ...
        try
        {
            ...
            //stmt.cUnexpected SQLException caught in NEW-ORDERlose();
            conn.close();
        }
        catch(Exception e)
        {
    ...
    private int deliveryTransaction(int w_id, int o_carrier_id)
    {
        ...
        try
        {
            ...
            for(d_id = 1; d_id <= 10; d_id++)
            {
		      ...
		      int a_100 = (int)(1+Math.random()*(100-1+1));
                      ...
                      delivDeleteNewOrder.setInt(1, a);
                      //delivDeleteNewOrder.setInt(2, w_id);
                      delivDeleteNewOrder.setInt(2, no_o_id);

		      delivDeleteNewOrder.executeUpdate();
                ...
                if(no_o_id != -1)
                {
                    ...
                    if (delivUpdateCustBalDelivCnt == null) {
                      delivUpdateCustBalDelivCnt = conn.prepareStatement(
                        "UPDATE benchmarksql.customer SET c_balance = -10 + ?" +
                        ", c_delivery_cnt = 0 + 1" +
                        " WHERE c_id = ?" +
                        " AND c_d_id = 1" +
                        " AND c_w_id = 1");
    ...
    private void orderStatusTransaction(int w_id, int d_id, int c_id, String c_last, boolean c_by_name)
    {
        ...
        try
        {
            if(c_by_name)
            {
                ...
                if (!rs.next()) {
                  log.error("ordStatGetCustBal() not found! C_ID=" + c_id + " C_D_ID=" + 1 + " C_W_ID=" + 1);
                }
        ...
        try {
            ...
            if (stmtGetCustWhse == null) {
              stmtGetCustWhse = conn.prepareStatement(
                ...
                " WHERE w_id = 1 AND w_id = c_w_id" +
                  " AND c_d_id = ? AND c_id = ?");
            }

            //stmtGetCustWhse.setInt(1, w_id);
            stmtGetCustWhse.setInt(1, d_id);
            stmtGetCustWhse.setInt(2, c_id);

            rs = stmtGetCustWhse.executeQuery();
            if (!rs.next()) {
              log.error("stmtGetCustWhse() not found! " + "W_ID=" + 1 + " C_D_ID=" + d_id + " C_ID=" + c_id);
            }
            ...
            while(!newOrderRowInserted)
            {
                ...
                //stmtGetDist.setInt(2, d_w_id);

                rs = stmtGetDist.executeQuery();
                if (!rs.next()) {
                  log.error("stmtGetDist() not found! " + "D_ID=" + d_id + " D_W_ID=" + 1);
                }
            ...
            if (result == 0) {
              log.error("stmtUpdateDist() Cannot update next_order_id on DISTRICT for D_ID=" + d_id + " D_W_ID=" + 1);
            }
		...
		int a = (int)(1+Math.random()*(100000-1+1));
                 ...
                 if ( ol_i_id == -12345) {
                  ...
                  if (stmtGetItem == null) {
                    stmtGetItem = conn.prepareStatement(
                      "SELECT i_price, i_name , i_data FROM benchmarksql.item WHERE i_id = 1 ");
                  }
                  //stmtGetItem.setInt(1, a);
                ...
                if (stmtGetStock == null) {
                  stmtGetStock = conn.prepareStatement(
                      ...
                      " FROM benchmarksql.stock WHERE s_i_id = 8 AND s_w_id = 1 ");
                }
                ...
                if (!rs.next()) {
                  log.error("stmtGetStock() not found! " + "I_ID=" + ol_i_id + " W_ID=" + 1);
                }
                ...
                if (stmtUpdateStock == null) {
                  stmtUpdateStock = conn.prepareStatement(
                    "UPDATE benchmarksql.stock SET s_quantity = ? , s_ytd = 0 + ?, s_remote_cnt = 0 + ? " +
    ...
    private void stockLevelTransaction(int w_id, int d_id, int threshold)
    {
        ...
        try
        {
              ...
              stockGetDistOrderId.setInt(1, d_id);

              rs = stockGetDistOrderId.executeQuery();
              if (!rs.next()) {
                log.error("stockGetDistOrderId() not found! D_W_ID=" + 1 + " D_ID=" + d_id);
              }
        ...
        try
        {
              ...
              if (payUpdateWhse == null) {
                payUpdateWhse = conn.prepareStatement(
                  "UPDATE benchmarksql.warehouse SET w_ytd = 30000 + ?  WHERE w_id = 1 ");
              }
              ...
              if (result == 0) {
                log.error("payUpdateWhse() not found! W_ID=" + 1);
              }
              ...
              if (!rs.next()) {
                log.error("payGetWhse() not found! W_ID=" + 1);
              }
              ...
              if (payUpdateDist == null) {
                payUpdateDist = conn.prepareStatement(
                  "UPDATE benchmarksql.district SET d_ytd = 30000 + ? WHERE d_w_id = 1 AND d_id = ?");
              }
              payUpdateDist.setFloat(1, h_amount);
              //payUpdateDist.setInt(2, w_id);
              payUpdateDist.setInt(2, d_id);
              result = payUpdateDist.executeUpdate();
              if (result == 0) {
                log.error("payUpdateDist() not found! D_ID=" + d_id + " D_W_ID=" + 1);
              }

              if (payGetDist == null) {
                payGetDist = conn.prepareStatement(
                  "SELECT d_street_1, d_street_2, d_city, d_state, d_zip, d_name" +
                  " FROM benchmarksql.district WHERE d_w_id = 1 AND d_id = ?");
              }

              //payGetDist.setInt(1, w_id);
              payGetDist.setInt(1, d_id);

              rs = payGetDist.executeQuery();
              if (!rs.next()) {
                log.error("payGetDist() not found! D_ID=" + d_id + " D_W_ID=" + 1);
              }
              ...
              // payment is by customer ID
                if (payGetCust == null) {
                  payGetCust = conn.prepareStatement(
                    ...
                    "       c_phone, c_credit, c_credit_lim, c_discount, c_balance, c_since " +
                    "  FROM benchmarksql.customer WHERE c_w_id = 1 AND c_d_id = 1 AND c_id = ?");
                }

                //payGetCust.setInt(1, c_w_id);
                //payGetCust.setInt(2, c_d_id);
                payGetCust.setInt(1, c_id);

                rs = payGetCust.executeQuery();
                if (!rs.next()) {
                  log.error("payGetCust() not found! C_ID=" + c_id + " C_D_ID=" + 1 + " C_W_ID=" + 1);
                }
            ...
            if(c_credit.equals("BC")) {  // bad credit
                ...
                if (payGetCustCdata == null) {
                  payGetCustCdata = conn.prepareStatement(
                    "SELECT c_data FROM benchmarksql.customer WHERE c_w_id = 1 AND c_d_id = 1 AND c_id = ?");
                }

                //payGetCustCdata.setInt(1, c_w_id);
                //payGetCustCdata.setInt(2, c_d_id);
                payGetCustCdata.setInt(1, c_id);

                rs = payGetCustCdata.executeQuery();
                if (!rs.next()) {
                  log.error("payGetCustCdata() not found! C_ID=" + c_id + " C_W_ID=" + 1 + " C_D_ID=" + 1);
                }
              ...
              if(c_new_data.length() > 500) c_new_data = c_new_data.substring(0, 500);
                ...
                if (payUpdateCustBal == null) {
                  payUpdateCustBal = conn.prepareStatement(
                    "UPDATE benchmarksql.customer SET c_balance = ? WHERE c_w_id = 1 AND c_d_id = 1 AND c_id = ?");
                }

                payUpdateCustBal.setFloat(1, c_balance);
                //payUpdateCustBal.setInt(2, c_w_id);
                //payUpdateCustBal.setInt(3, c_d_id);
                payUpdateCustBal.setInt(2, c_id);

                result = payUpdateCustBal.executeUpdate();
                if (result == 0) {
                  log.error("payUpdateCustBal() not found! C_ID=" + c_id + " C_W_ID=" + 1 + " C_D_ID=" + 1);
                }
            ...
            int a_hist_id = (int)(1+Math.random()*(100000000-1+1));
	    if (payInsertHist == null) {
              payInsertHist = conn.prepareStatement(
                "INSERT INTO benchmarksql.history (hist_id,h_c_d_id, h_c_w_id, h_c_id, h_d_id, h_w_id, h_date, h_amount, h_data) " +
                " VALUES (30000 + ?,?,?,?,?,?,?,?,?)");
            }

            payInsertHist.setInt(1, a_hist_id);
	    payInsertHist.setInt(2, c_d_id);
            payInsertHist.setInt(3, c_w_id);
            payInsertHist.setInt(4, c_id);
            payInsertHist.setInt(5, d_id);
            payInsertHist.setInt(6, w_id);
            payInsertHist.setTimestamp(7, new Timestamp(System.currentTimeMillis()));
            payInsertHist.setFloat(8, h_amount);
